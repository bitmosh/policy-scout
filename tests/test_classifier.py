...
def test_package_install_classification():
...
def test_package_execute_classification():
...
def test_network_execute_classification():
...
def test_credential_adjacent_classification():
    """Test classification of credential-adjacent commands."""
    parser = ShellParser()
    classifier = setup_classifier()

    commands = ["cat .env", "cat ~/.ssh/id_rsa", "cat ~/.npmrc", "grep -r TOKEN ."]
...
def test_destructive_classification():
    ...
    commands = ["rm -rf /", "rm -rf ~", "find . -type f -delete", "git clean -fdx"]
...
def test_inspection_classification():
